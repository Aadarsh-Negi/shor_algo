    ...
    def SetMap(self, toRegister, mapping, propagate = True):
        ...
        for x, yStates in mapTensorX.items():
            for y, element in yStates.items():
                ...
                toState.SetEntangled(fromState, amplitude)
                fromState.SetEntangled(toState, amplitude.conjugate())
        if propagate:
            toRegister.SetPropagate(self)
...
Nx = int(input("Enter a number - "))
results_algo = ExecuteShors(Nx, 10, 0.01, 2)
